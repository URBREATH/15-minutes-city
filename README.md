...
1. The street network is downloaded from OSM through the **Pandana library**, limited to pedestrian or bicycle-accessible streets. If a network file already exists in the output_path folder, it is reused and not downloaded again.The same logic is applied to POIs.
...
| `ERR_010`  | Invalid parameter value park_gates_source: `osm \| csv \| road_network \| virtual` |
| `ERR_011`  | park_gates_csv missing or invalid |
| `ERR_012`  | poi_category_custom_csv not found `<poi_category_custom_csv>` |
| `ERR_013`  | clip_layer_path not found `<clip_layer_path>` |
| `ERR_014`  | grid_path not found `<grid_path>` |
