...
from scripts.storage_minio import check_path_exists,check_folder_exists,split_path,get_s3_client,minio_file_exists,minio_copy_prefix,minio_list_poi_categories,is_minio_path,split_bucket_and_prefix,load_pois_from_minio
    ...
    if network_nodes:
        if is_minio_path(network_nodes):
            ...
            if not os.path.isfile(network_nodes):
                raise_error("ERR_021", extra=network_nodes)
    ...
    if network_edges:
        if is_minio_path(network_edges):
            ...
            if not os.path.isfile(network_edges):
                raise_error("ERR_022", extra=network_edges)   
